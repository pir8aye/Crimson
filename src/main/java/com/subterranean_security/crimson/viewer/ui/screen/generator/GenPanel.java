...
public class GenPanel extends JPanel {
	...
	public ClientConfig getValues() {
		...
		ic.setViewerUser(ViewerProfileStore.getLocalViewer().get(AKeySimple.VIEWER_USER));
	...
	public boolean testValues(ClientConfig config) {
		...
		// test paths
		if (!config.getPathWin().isEmpty() && !ValidationUtil.path(config.getPathWin())) {
			lbl_status.setBad("Invalid Windows install path");
			return false;
		}

		if (!config.getPathLin().isEmpty() && !ValidationUtil.path(config.getPathLin())) {
			lbl_status.setBad("Invalid Linux install path");
			return false;
		}

		if (!config.getPathOsx().isEmpty() && !ValidationUtil.path(config.getPathOsx())) {
			lbl_status.setBad("Invalid OSX install path");
			return false;
		}

		if (!config.getPathSol().isEmpty() && !ValidationUtil.path(config.getPathSol())) {
			lbl_status.setBad("Invalid Solaris install path");
			return false;
		}

		if (!config.getPathBsd().isEmpty() && !ValidationUtil.path(config.getPathBsd())) {
